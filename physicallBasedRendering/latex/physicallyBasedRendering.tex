...
\cite{openGlWolf,learnOpenGL}% Einleitung S.133
\cite{learnOpenGL}
\cite{reasonsForPbr1,reasonsForPbr2,pbrGuide,learnOpenGL} % Vorteile PBR
\cite{Beyerer2016} %einleitung + strahlungsfluss S.179 f.
\cite{Beyerer2016} %einleitung + strahlungsfluss S.179 f.
\cite{learnOpenGL, irrlichtOrg} %Raumwinkel irrlicht S.2
\cite{GrundlagenLichtTechnik,radiometrischeGrundbegriffe,vgb,learnOpenGL} %strahldichte S.8f. S.36f.
\cite{gMueller} %strahldichte
\cite{GrundlagenLichtTechnik}%besttrahlungsstärke S.10 
\cite{GrundlagenLichtTechnik}%besttrahlungsstärke S.10
\cite{GRAY197813,Ryer97lightmeasurement,renderingEquationArticle} %Labertsches Kosinus gesetz S.21f.
\cite{learnOpenGL} 
...
\cite{irrlichtOrg,uniUlm} %BRDF Standard S.1 S.2
\cite{irrlichtOrg} %S.3
\cite{realTimeRendering4th,learnOpenGL,uniUlm,irrlichtOrg} %gesetze  S.312f. S.2 S.3
\cite{irrlichtOrg,pbrGuide,learnOpenGL,googleFilamentPbr,realTimeRendering4th,rtrPaper} %Reflactance S.5 S.305f. u. 316f. S.6 u. 11
\cite{pbrGuide,learnOpenGL,googleFilamentPbr,rtrPaper} %Reflactance S.8
\cite{learnOpenGL}
\cite{realTimeRendering4th,irrlichtOrg,learnOpenGL}%Lambert Diffuse S.313f. S.5f. 
\cite{realTimeRendering4th,learnOpenGL}%Lambert Diffuse S.314
\cite{learnOpenGL}%Lambert Diffuse
\cite{learnOpenGL}%Cooktorrance
\cite{learnOpenGL}%
\cite{learnOpenGL,rtrPaper,realTimeRendering4th} %Mikrofacett MOdell S.12 S.331f.
\cite{rtrPaper}
\cite{learnOpenGL,rtrPaper,realTimeRendering4th} %NDF S.15f. S.332f. u. 337f.
\cite{learnOpenGL}
\cite{learnOpenGL}
\cite{Burley2012PhysicallyBasedSA} %Disney alpha 
\cite{learnOpenGL}
\cite{rtrPaper,irrlichtOrg}% Geometry S.12f. S.9
\cite{rtrPaper} % S.16
\cite{learnOpenGL}
\cite{learnOpenGL}
\cite{realTimeRendering4th,rtrPaper}%fresnel S.316ff. s.13f.
\cite{realTimeRendering4th,rtrPaper,learnOpenGL}%fresnel S.321 S.13
\cite{learnOpenGL}
\cite{learnOpenGL}
